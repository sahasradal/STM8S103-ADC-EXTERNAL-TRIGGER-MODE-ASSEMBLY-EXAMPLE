stm8/
	; works fine . A high edge on PB4 will trigger a single shot conversion of signal on AIN2 = PC4
	; 9600 BAUD UART TX on PD5
	#include "mapping.inc"
...
ADC_INIT:
	mov ADC_CR2,#$58	; ext select bits 5:4 = 01, External interrupt on ADC_ETR pin PB4.bits 6,4,3
	mov ADC_CR1,#$21    ; ADC prscaler is clk/4 = 010 ; ADON ,0b01000001
	mov ADC_CSR,#$02 	; select channel-2 ,  PC4
...
con_not_finish:
	...
	ld a,ADC_DRL		; copy ADC higher result register to A
	ld ADCresult5,a		; store LSB in ADCresult5
	ld a,ADC_DRH		; copy ADC lower result register to A 
...
ADC2_WRITE:	
	...
	jp ADC_READ			; jump to ADC_READ label an wait for external  trigger on PB4
